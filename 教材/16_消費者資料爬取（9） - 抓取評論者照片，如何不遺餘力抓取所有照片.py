# %% [markdown]
# # 16_消費者資料爬取（9） - 抓取評論者照片，如何不遺餘力抓取所有照片
...
for ar in all_reviews:
    ...
    photo_urls=[]
    for ph in photos:
        photo_urls.append(ph.get('style').???('"background-image: url(').split('")')[0]
    all_photo_review.append(photo_urls)

school=pd.DataFrame({"學校名稱":???,"評論者名稱":all_title_review,"評論時間":all_time_review,"評論內容":all_text_review,"評論星數":all_star_review,"評論照片":all_photo_review})
school.head()
