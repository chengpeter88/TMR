# %% [markdown]
# # 14_消費者資料爬取（7） - 抓取評論者星評，好壞評狀況一覽無遺
...
# %%
all_star_review.append(str(ar.find(class_ = "kvMYJc").get('aria-label')))
...
# %%
all_star_review[0].strip().strip(" 顆星")
...
# %%
school=pd.DataFrame({"學校名稱":search,"評論者名稱":all_title_review,"評論時間":all_time_review,"評論內容":all_text_review,"評論星數":all_star_review})
school

browser.close()
# %% [markdown]
# ## 小練習
# ### 請嘗試爬取上堂課指定題目 search="北商" 下拉4次 scroll=4 的所有評論、評論時間和評論星數(需先執行下面段落)
...
#建立dataframe
school=pd.DataFrame({"學校名稱":search,"評論者名稱":all_title_review,"評論時間":all_time_review,"評論內容":all_text_review,"評論星數":???})
school
