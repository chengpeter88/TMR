# %% [markdown]
# # 11_消費者資料爬取（4） - 揭秘爬取多頁數評論技巧，搜羅更完整的評論資料
...
# %% [markdown]
# ## 小練習
# ### 請嘗試爬取上堂課指定題目 search="北商" 下拉4次 scroll=4 的所有評論(需先執行下面段落)
...
for st in range(???):
    time.sleep(2)
    pane = browser.find_element_by_xpath('//*[@id="QA0Szd"]/div/div/div[1]/div[2]/div/div[1]/div/div/div[2]')
    browser.???("arguments[0].scrollTop = arguments[0].scrollHeight", pane)
    print(st+1,'scroll')
    time.sleep(2)
...
all_title_review=[]
for ar in all_reviews:
    all_title_review.append(ar.find(class_ = "d4r55").???)
    try:
        all_text_review.append(ar.find(class_ = "wiI7pd").???)
    except:
