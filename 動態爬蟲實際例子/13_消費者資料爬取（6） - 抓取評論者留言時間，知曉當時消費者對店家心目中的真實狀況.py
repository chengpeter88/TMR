# %% [markdown]
# # 13_消費者資料爬取（6） - 抓取評論者留言時間，知曉當時消費者對店家心目中的真實狀況
...
# %%
school=pd.DataFrame({"學校名稱":search,"評論者名稱":all_title_review,"評論時間":all_time_review,"評論內容":all_text_review})
school
...
all_reviews = soup.???(class_ = 'jftiEf fontBodyMedium')
...
for ar in all_reviews:
    ...
    all_title_review.append(ar.find(class_ = "d4r55").text)
    all_time_review.append(ar.find(class_="???").text)
